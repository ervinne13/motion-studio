...
function buildExtend(wf, { imageFilename, videoFilename, fps, frameCount, startFrame, prompt, seed, prefix, prevVideoFile, condFrames, replacementMode }) {
  const w = structuredClone(wf);
...
export async function generateSegment({
  ...
  if (isBase) {
    ...
    workflow = buildExtend(extendWF, {
      ...
      prevVideoFile: condVideoFilename, condFrames, replacementMode,
    });
  }
