...
import { uploadImage, uploadVideo, queuePrompt, waitForResult, downloadOutput } from './comfyui.js';
...
function buildBase(wf, { imageFilename, videoFilename, fps, frameCount, startFrame, prompt, seed, prefix }) {
...
function buildExtend(wf, { imageFilename, videoFilename, fps, frameCount, startFrame, prompt, seed, prefix, prevVideoFile }) {
  const w = structuredClone(wf);
...
export async function generateSegment({
  ...
  if (segmentIndex === 0) {
    ...
    workflow = buildExtend(extendWF, {
      imageFilename, videoFilename, fps: genFps,
      frameCount, startFrame, prompt, seed, prefix: jobPrefix,
      prevVideoFile: prevComfyFilename,
    });
  }
  ...
  // Re-upload to ComfyUI input so the next segment's extend workflow can read it
  const comfyInputFilename = await uploadVideo(outputPath);
