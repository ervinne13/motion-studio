...
let _tickEpoch       = 0;    // incremented by watchdog to invalidate a stuck tick's lock
...
async function tick() {
  ...
  if (_tickLock) return;
  _tickLock = true;
  const myEpoch = ++_tickEpoch;
  try {
    await _tickBody();
  } finally {
    // Only release the lock if the watchdog hasn't already stolen it (epoch mismatch)
    if (_tickEpoch === myEpoch) _tickLock = false;
  }
}
...
async function _tickBody() {
  ...
  const watchdog = setInterval(async () => {
    ...
    try {
      ...
      if (queueRunning === 0 && queuePending === 0) {
        ...
        // Steal the tick lock: bump epoch so the old stuck tick's finally won't re-acquire it,
        // then release immediately so the next tick() can proceed.
        _tickEpoch++;
        _tickLock = false;
        setTimeout(tick, 300);
      }
  ...
  try {
    ...
    if (job.params?.jobType === 'qwen-edit') {
    ...
    } else if (job.params?.jobType === 'rife-2x' || job.params?.jobType === 'rife-segment') {
      ...
      // Pick best available per-segment file: rife > esrgan > original
      const inputs = await Promise.all(genSegs.map(async seg => {
        const orig        = join(genDir, seg.generatedVideo);
        const esrganPath  = orig.replace(/\.mp4$/i, '_2x.mp4');
        const rifePath    = orig.replace(/\.mp4$/i, '_rife.mp4');
        const esrganRife  = orig.replace(/\.mp4$/i, '_2x_rife.mp4');
        for (const p of [esrganRife, rifePath, esrganPath]) {
    ...
    const current = await findJob(job.id);
    if (!['cancelled', 'done', 'failed'].includes(current?.job?.status)) {
    ...
    clearInterval(watchdog);
    if (_processingJobId === job.id) _processingJobId = null;
    setTimeout(tick, 300);
  }
}
    ...
    for (const [fp, jobs] of pairs) {
      ...
      for (const job of jobs) {
        if (job.status === 'pending' || job.status === 'waiting') {
          ...
          dirty = true;
          count++;
          log(`[queue] cancel-all-pending: cancelled job ${job.id}`);
        }
      }
...
// Returns all jobs (any status) for a specific project, across all log files
export async function getJobsForProject(projectId) {
  const results = [];
  for (const [, jobs] of await allLogFiles()) {
    results.push(...jobs.filter(j => j.params?.projectId === projectId));
  }
  return results;
}
