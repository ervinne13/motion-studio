...
async function tick() {
  ...
  try {
    let result;
    const onQueued = async (promptId) => {
      log(`[queue] job ${job.id} submitted to ComfyUI as prompt ${promptId}`);
      await patchJob(job.id, { comfyPromptId: promptId });
    };
    ...
    if (!['cancelled', 'done'].includes(current?.job?.status)) {
      const isTransient = /timed out|fetch failed|ECONNREFUSED|ECONNRESET|network/i.test(err.message);
