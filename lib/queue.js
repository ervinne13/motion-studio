...
async function tick() {
  ...
  if (!free) {
    ...
    // ComfyUI is busy but we have no local job running — zombie prompt from a
    // previous server session. Interrupt it so this job can proceed.
    if (!_processingJobId && running > 0) {
      log(`[queue] ComfyUI running zombie prompt (no local job active) — sending interrupt`);
      try {
        await fetch(`${COMFYUI}/interrupt`, { method: 'POST' });
      } catch { /* ignore — next tick will re-check */ }
    }
    return;
  }
  ...
  try {
    ...
    if (job.params?.jobType === 'qwen-edit') {
      result = await generateQwenEdit(job.params);
    } else {
