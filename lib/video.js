...
// Mix audio from audioSourcePath into silent videoPath → outputPath.
// Uses -shortest so the output ends when the shorter stream ends.
export async function mixAudio(videoPath, audioSourcePath, outputPath) {
  return new Promise((resolve, reject) => {
    const proc = spawn('ffmpeg', [
      '-y',
      '-i', videoPath,
    ...
    proc.on('close', code => code === 0 ? resolve() : reject(new Error(`ffmpeg audio mix exited ${code}`)));
    proc.on('error', reject);
  });
}
...
export async function hasAudioStream(videoPath) {
  return new Promise(resolve => {
    ...
    proc.stdout.on('data', d => out += d);
    proc.on('close', () => {
      try { resolve((JSON.parse(out).streams ?? []).length > 0); }
      catch { resolve(false); }
    });
...
// Re-encode a video to a target frame rate (used to align clip FPS with project genFps).
export async function convertFps(inputPath, outputPath, targetFps) {
  return new Promise((resolve, reject) => {
    const proc = spawn('ffmpeg', [
      '-y', '-i', inputPath,
    ...
    proc.on('close', code => code === 0 ? resolve() : reject(new Error(`ffmpeg fps convert failed (${code})`)));
    proc.on('error', reject);
  });
}

// Split a video at splitFrame into left (frames 0..splitFrame-1) and right (splitFrame..end).
export async function splitVideoAtFrame(inputPath, leftPath, rightPath, splitFrame) {
  await new Promise((resolve, reject) => {
    const proc = spawn('ffmpeg', [
      '-y', '-i', inputPath,
      '-vf', `trim=start_frame=0:end_frame=${splitFrame},setpts=PTS-STARTPTS`,
      '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-an',
      leftPath,
    ], { stdio: 'ignore' });
    proc.on('close', code => code === 0 ? resolve() : reject(new Error(`ffmpeg split-left failed (${code})`)));
    proc.on('error', reject);
  });
  await new Promise((resolve, reject) => {
    const proc = spawn('ffmpeg', [
      '-y', '-i', inputPath,
      '-vf', `trim=start_frame=${splitFrame},setpts=PTS-STARTPTS`,
      '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-an',
      rightPath,
    ], { stdio: 'ignore' });
    proc.on('close', code => code === 0 ? resolve() : reject(new Error(`ffmpeg split-right failed (${code})`)));
    proc.on('error', reject);
  });
}
...
export async function concatVideos(inputPaths, outputPath) {
  ...
  const listContent = inputPaths.map(p => `file '${resolve(p).replace(/'/g, "'\\''")}'`).join('\n');
